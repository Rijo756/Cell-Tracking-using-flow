...
parser.add_argument('--out_path',type=str, default="Script Path / output",help="Will generate during the run")

parser.add_argument('--batch_size',type=int, default= 2, help="Size of the batch, for deeptracking it is 1")
parser.add_argument('--lr',type=float, default=0.01, help="learning rate")
parser.add_argument('--iter',type=int, default= 81 , help="Number of epochs")
...
def plot_loss(args, epochs, trainloss, validationloss, label1 ='Training loss', label2 = 'validation loss', name = 'training_progress'):
    ...
    if args.steps <= 1:
        plt.legend()
    plt.savefig(os.path.join(args.out_path,"UnetSegmentation_aug_"+ name +".png"))

def watershed_algorithm(segmentation):
    pass
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(script_path,"Train_Segment")
    ...
    dataset = FlowDataset(base_folder = args.image_path, subfolder_pattern = args.subfolder_pattern , loghandler = loghandler, time_skips = [1,2,3,4,5], transforms = transforms )
    ...
    model = UNetArchitecture(num_classes = 1, in_channels= 1 , depth= 3)
    ...
    for iter in range(args.iter):
        ...
        if iter % 10 == 0:
            torch.save(model.state_dict(), os.path.join(args.out_path,"seg_network_trained_Hela_aug_"+str(iter)+".pth") )
            loghandler.log('info', "Model Saved in ../seg_network_trained_Hela_aug_"+str(iter)+".pth ")
